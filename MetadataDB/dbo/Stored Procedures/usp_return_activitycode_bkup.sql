...
CREATE procedure [dbo].[usp_return_activitycode_bkup] 
@PipelineID Int
as



--declare @Childactivity_code table (ID int IDENTITY(1,1),ActivityJsoncode varchar(max),PipelineActivityId INT)
--INSERT INTO @Childactivity_code
--Select Case when RowNumber =2 Then ','+Code else Code end as Code,PipelineActivityId 
--from (
--select row_number() over(order by ChildTLS.code) as RowNumber, ChildTPS.ID as PipelineActivityId
--, replace(ChildTLS.code,'$','$'+ISNULL(ChildTPS.ActivityName,ChildTLS.ActivityStandardName)+'_') as Code 
--,ChildTPS.ID
...
--join [dbo].[T_Pipelines_Steps] ChildTPS on ChildTPS.PipelineiD = TP.id
--join [dbo].[T_List_Activities] ChildTLS on ChildTLS.ID = ChildTPS.Child_Activity
--WHERE TP.ID =  and ChildTPS.Child_Activity is not null
--)C
...
insert into @activity_code select ', {
                "name": "Execute Send Mail for '+ISNULL(PS.Activityname,LS.ActivityStandardName)+'",
